...
//******************************************************************************
    ...
    // added zero extending for ANDI/ORI/XORI
    wire [31:0] imm_zero_extend = {16'b0, immediate};
    wire [31:0] imm_sign_extend = {{16{immediate[15]}}, immediate};
    wire [31:0] imm_upper = {immediate, 16'b0};

    wire isZeroExtImm = (op == `ANDI) || (op == `ORI) || (op == `XORI);

    wire [31:0] imm = (op == `LUI) ? imm_upper :  
                      (isZeroExtImm) ? imm_zero_extend :
                      imm_sign_extend;
...
//******************************************************************************
    ...
    // added the read_from_rt signal because sometimes rt is used as dest and not src
    
    // EX->ID forwarding for rs and rt
    wire forward_rs_ex = &{rs_addr == reg_write_addr_ex, rs_addr != `ZERO, reg_we_ex};
    wire forward_rt_ex = &{rt_addr == reg_write_addr_ex, rt_addr != `ZERO, reg_we_ex, read_from_rt};

    // assignment
    // note: ex data assignment goes first b/c it is more recent (holds newest data)
    assign rs_data = 
...
//******************************************************************************
    ...
    wire [31:0] pc_plus_8 = pc + 32'd8;
    assign alu_op_y = (isJAL | isJALR) ? pc_plus_8 :
                      (use_imm) ? imm : rt_data;
    assign reg_write_addr = isJAL ? `RA :
